...
In order to utilize the functionality provided by [`auto_running/auto_run.py`](./auto_running/auto_run.py), one must pass in a JSON parameter file. These parameters tell the program information about how to interact with your SCFT trial (such as which directory you are storing your calculations in). While the program does provide a default template parameter file, [`auto_running/param.json`](./auto_running/param.json), users may want or need to change certain parameters. This section describes all required JSON parameters for `auto_run.py`. For more information on what `auto_run.py` can do, see [Automatic Execution Implementation](#automatic-execution-implementation). As there are a large amount of parameters required for `auto_run.py` and users will typically only need/want to change a few of them, `auto_run.py` allows you to pass an infinite number of parameter files. The parameters of earlier files will be preserved, but any additional files will override existing parameters (see `deep_merge()` in [Modifying Automatic Execution (For Developers)](#modifying-automatic-execution-for-developers) for more information on how this is accomplished). Therefore, it is possible for a user to pass in the default `auto_running/param.json` as well as another JSON file containing just the parameters that they seek to override.
**NOTE**: The first argument passed in after the program name will be used as the file to write logging information to, using Python's `logging` module. Therefore, an example full command to run `auto_run.py` could be `python auto_run.py log.txt param.json custom.json`\
